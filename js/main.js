      ...
      const gammesInterne = ordreGammes.map(gamme => {
        ...
        return `
          <div class="ligne-groupe" data-gamme="${gam_id}">
            ...
            <div class="produits-grille">${photoGamme ? `<div class="bloc-gamme-photo"><img src="${photoGamme}" alt="${gamme}" onerror="this.parentElement.style.display='none'"></div>` : ''}${prods.map(p => carteProduit(p)).join('')}</div>
          </div>`;
      }).join('');
